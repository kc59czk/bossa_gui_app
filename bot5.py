...
class BossaApp:
    ...
    def cancel_selected_order(self):
        ...
        # Zbierz wszystkie potrzebne dane z wiersza
        order_details = {
            'id_dm': item_values[0],
            'k_s_text': item_values[4], # "Kupno" lub "Sprzedaż"
            'ilosc': item_values[5],
            'rachunek': self.account_entry.get() # Pobieramy z pola w GUI
        }
        
        if not order_details['rachunek']:
            messagebox.showerror("Błąd", "Nie można anulować zlecenia bez podanego numeru rachunku w zakładce 'Zlecenie Ręczne'.")
            return
    ...
    # ... (pozostałe metody BossaApp bez zmian)
...
class BossaAPIClient:
    ...
    def cancel_order(self, order_details):
        ...
        # Konwersja "Kupno"/"Sprzedaż" na '1'/'2'
        side = '1' if order_details['k_s_text'] == "Kupno" else '2'

        # Zgodnie z dokumentacją, podajemy więcej szczegółów
        fixml_request = f"""<FIXML v="5.0" r="20080317" s="20080314">
<OrdCxlReq ID="{client_cancel_id}" OrdID="{order_details['id_dm']}" Acct="{order_details['rachunek']}" Side="{side}">
...
<OrdQty Qty="{order_details['ilosc']}"/>
    ...
    def _parse_execution_report(self, xml_data):
        try:
            # ... (logika parsowania dla GUI Treeview bez zmian)
            ...
            # --- POPRAWIONA LOGIKA BOTA ---
            # Przetwarzaj logikę tylko dla zleceń wykonanych ('2')
            if status == '2':
                last_px_str = exec_rpt.get('LastPx')
                # Sprawdź, czy cena istnieje, zanim spróbujesz jej użyć
                if not last_px_str:
                    return
