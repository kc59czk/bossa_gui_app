...
<InstReq><Instrmt ID="{isin}" Src="4"/></InstReq>
        ...
        if response and '<MktDataFull' in response:
            ...
            self.subscribed_isins[isin] = {"last_price": 0.0, "last_update": datetime.now()}
            return True
        else:
    ...
    def _process_market_data(self, xml_data):
        """Process market data increments to extract last transaction prices."""
        try:
            ...
            for mkt_data_inc in root.findall('MktDataInc'):
                for instrmt in mkt_data_inc.findall('Instrmt'):
                    ...
                    if isin in self.subscribed_isins:
                        # Look for last trade price (Type="2")
                        for entry in mkt_data_inc.findall('Entry'):
                            if entry.get('Typ') == "2":  # Last trade price
                                price = entry.get('Px')
                                if price:
                                    try:
...
class ScalpingBotApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Warsaw Stock Exchange Scalping Bot with Real Market Data")
